# ACL-Adaptive-RAG-for-Multi-Hop-Question
Below is a more detailed English description of the repository structure and the purpose of each script, suitable for a README or project documentation.
...
|---------------|-------------|
| `qwen_3_5_validation.ipynb` | Scripts to compute **entropy scores** on a validation set for open‑source models (Qwen3.5‑4B, Qwen3.5‑9B, Llama 3.1 8B, Gemma 4B). Outputs are used to select the optimal entropy threshold $\tau^*$ (Youden’s index). |
...
- **Adaptive RAG**  
  ...
  - Threshold $\tau$ is calibrated per model on the validation set.
